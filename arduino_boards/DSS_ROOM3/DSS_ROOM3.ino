...
char ssid[] = "Telecanal2_lmrqht";    // Your WiFi network SSID (name)
char pass[] = "37WX43XX";             // Your WiFi network password
WiFiClient client;                      // Create a WiFi client instance for network communication

// ThingSpeak channel credentials
unsigned long myChannelNumber = 2110036;         // Your ThingSpeak channel number
const char *myWriteAPIKey = "R88IGVXC87IR1N8B";  // Your ThingSpeak API Key
...
#define SteamSensorPin 2    // Analog pin for Steam Sensor
#define COSensorPin 5    // Analog pin for CO Sensor
...
float currentSteam = 0;
float steamAccumulator = 0;
float averageSteam = 0;
float currentCO = 0;
float COAccumulator = 0;
float averageCO = 0;
...
void loop() {
  ...
  steamAccumulator += currentSteam;
  COAccumulator += currentCO;
...
void readSensorData() {
  ...
  currentSteam = analogRead(SteamSensorPin);
  currentCO = analogRead(COSensorPin);
...
void calculateAverages() {
  ...
  averageSteam = steamAccumulator / sampleCount;
  averageCO = COAccumulator / sampleCount;
}
...
void sendDatatoThingSpeak(){
  if (debug) {
    ...
    Serial.print("Average Steam: ");
    Serial.println(averageSteam);
  ...
  ThingSpeak.setField(3, PIRMotionAccumulator);
  ThingSpeak.setField(4, averageSteam);
  ThingSpeak.setField(5, averageCO);
